...
function findFirstSingleChar(str) {
  return str.split('').filter((x, _, arr) => arr.indexOf(x) === arr.lastIndexOf(x)).shift();
}
...
function getDigitalRoot(num) {
  const res = +num.toString().split('').reduce((result, item) => +result + +item, 0);
  if (res > 9) {
...
function isBracketsBalanced(str) {
  ...
  const closing = Object.values(basic);
  const stack = [];

  for (let i = 0; i < str.length; i += 1) {
    if (basic[str[i]]) {
      stack.push(str[i]);
    } else if (closing.includes(str[i]) && str[i] !== basic[stack.pop()]) {
      return false;
    }
  }
  return !stack.length;
